...
This is the architectural step up from [`finagent`](../finagent):
`finagent` is one orchestrator calling three tools in a hardcoded
...
| Repo | Type | Role |
|---|---|---|
| [`finsent-tf`](../finsent-tf) | Discriminative NLP | Sentiment scoring |
| [`rag-financial-qa`](../rag-financial-qa) | Generative / RAG | Retrieval-augmented Q&A |
| [`finforecast-tf`](../finforecast-tf) | Time-series | Direction forecasting |
| [`finagent`](../finagent) | Single-agent orchestration | Fixed-sequence tool calling |
...
- **Mock-first data tools, LLM-required reasoning.** Sentiment,
  ...
  point of the project — so those three nodes call a real Anthropic
  model via `langchain-anthropic`.
...
Same pattern as `finagent`: `src/tools/sentiment_tool.py`,
`research_tool.py`, and `forecast_tool.py` are mock-first with a
documented swap-in point for the real `finsent-tf`,
`rag-financial-qa`, and `finforecast-tf` repos (see `finagent`'s
...
│   ├── llm.py                    # real Anthropic client + FakeLLM for tests
